import sys, requests, os,functools
...
def buttonClick():
    ...
    if mainWindow.sender().text() == '交流':
        webbrowser.open("https://space.bilibili.com/394281846")
...
def tablistClick():
    ...
    :return:
    '''
    global dangqianData, spjzFrom, zrzwFrom,zbfxFrom,zqgdFrom
...
def zidong_kuanjia(chessData, i):
    ...
    tp_qlb.setStyleSheet('''#tp_qlb{padding: 1px;  border: 2px solid %s;  }''' % color)
    title = QLabel()
...
def mainWindowInitialize():
    ...
    mainWindow.move(0, 0)
    mainWindow.setWindowTitle('懒人TFT阵容查看')
    ...
    mainWindow.enterEvent=functools.partial(enterEvent,window=mainWindow)


    ui.bt_zdlb.setToolTip('<b>提示:</b><br>1.支持多选和Ctrl+A全选,选中多个后可以按Del键删除<br>2.双击删除指定英雄<br>3.点击英雄左上方的灯泡可以暂时不点它')
def enterEvent(e,window):
    # 鼠标移动事件 事件参数是返回的Event
...
def straFormInitialize():
    ...
    straForm.enterEvent = functools.partial(enterEvent, window=straForm)
...
def squadListInitialize():
    ...
    squadList.enterEvent = functools.partial(enterEvent, window=squadList)
...
def zdListInitialize():
    ...
    zdlistForm.enterEvent = functools.partial(enterEvent, window=zdlistForm)
    # 从文件中加载之前的记录
    try:
...
def showShouPaiJieZou():
    ...
    if dangqianData != '':
        ...
        spjzFrom.enterEvent = functools.partial(enterEvent, window=spjzFrom)
...
def showZhengRongZW():
    ...
    if dangqianData != '':
        ...
        zrzwFrom.move(app.desktop().screenGeometry().width() - zrzwFrom.geometry().width(),
                      straForm.geometry().height())
        zrzwFrom.enterEvent = functools.partial(enterEvent, window=zrzwFrom)
...
def showZaoQiGD(kg=False):
    ...
    if dangqianData != '':  # 如果数据正常的话继续执行
        ...
        zqgdFrom.enterEvent = functools.partial(enterEvent, window=zqgdFrom)
...
def showZhuangBeiFX(kg=False):
    ...
    if dangqianData != '':
        ...
        try:
            ...
            zbfxFrom.move(app.desktop().screenGeometry().width() - zbfxFrom.geometry().width(),
                          straForm.geometry().height())
        zbfxFrom.enterEvent = functools.partial(enterEvent, window=zbfxFrom)
